...
def rep_cluster(seq_path, path_merge_bed, path_rep, path_contig, fullContigPath):
    seq = {}
    #下面这里的代码写的有点迷,至少从看起来是需要我们提供contig的具体的信息的。
    # 另外，SeqIO的输入路径不能带空格
    ...
    for line in f.readlines():
        ...
        contigs = line[3]
        contigs = contigs.split(',') #因为我们merge过了，所以不同的contig之间使用，来分隔的。
        contig_name1 = []
        for contig_info in contigs:
            ...
            for num in range(2, len(name) - 1):
                ...
                sample_name += '_'
            sample_name += name[len(name) - 1]  # 从代码的情况来推测，这里可能是把很多的东西整合到了一起。团到了一起
            '''                                    # 应该不仅仅是它的代码中写的那样就只是把sample的一些信息团到一起，应该是把contig的
                                                # 信息都给写到了一起，用 _ 链接起来。 然后这里在给他分开来穿起来。
        ...
        contig_name = sorted(contig_name1, key=(lambda x: x[0]), reverse=True) # 这个还挺有意思的，这个和c++的sort异曲同工，大同小异。
